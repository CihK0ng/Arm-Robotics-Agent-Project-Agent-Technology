...
public class PickRum : MonoBehaviour
{
    ...
    [SerializeField] private IsTwoArmCollider _isTworumcollider;

    private GameObject getob;
    
    public IEnumerator Pickrumprocees()
    {
        Debug.Log("Toi di chuyen chai ruou");
        ...
        Debug.Log("Ban lay chai ruou n");
        yield return  new WaitForSeconds(1f);
    }
<<<<<<< HEAD:My project/Assets/script/PickRum.cs
...
>>>>>>> 1bbde39e728a9fed335b21e1f28a12d7cea7f598:My project/Assets/PickRum.cs
    ...
    IEnumerator MoveAndPlace(GameObject __rum, GameObject __forkpoint, GameObject __arm, GameObject __endpoint)
    {
        ...
        yield return new WaitUntil(() => __rum.transform.parent != null);

        yield return new WaitForSeconds(2f);

    }

    public IEnumerator CheckCollisionAndProceed()
    {
        // Gọi hàm kiểm tra va chạm
        _isTworumcollider.CheckTwoArmCollider();

        // Đợi cho đến khi điều kiện đúng
        yield return new WaitUntil(() => _isTworumcollider.Getvaluecollider() == true);

        // Thực hiện các hành động cần thiết sau khi va chạm
        // Kết thúc Coroutine
        yield return null;
    }
    // Di chuyen 
    ...
    {
        while (Mathf.Abs(_endDistance - _forkDistance) > 0.1f)
        {
            //Debug.Log("aa");
            if (_endDistance > _forkDistance)
            {
                __arm.transform.localPosition += new Vector3(0, 0, 0.06f);
