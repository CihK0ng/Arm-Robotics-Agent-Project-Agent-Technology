...
public class PickAndPlaceTai : MonoBehaviour
{
    ...
    {
        // EndPointControll AddEndPoint = GetComponent<EndPointControll>();


        _findboxaround.FindCubesAround(_ar.transform.position);
        receivedList = _findboxaround.GetList();
        Debug.Log(receivedList[0]);
        yield return StartCoroutine(MoveAndPick(receivedList[0], _forkob, _arm, _endPoint));
        yield return StartCoroutine(MoveAndPlace(receivedList[0], _forkob, _arm, _endPoint));

        _endPoint.position += new Vector3(0, 2f, 0);
        Debug.Log(_endPoint.position);
        yield return StartCoroutine(MoveAndPick(receivedList[1], _forkob, _arm, _endPoint));
        yield return StartCoroutine(MoveAndPlace(receivedList[1], _forkob, _arm, _endPoint));
